...
used to estimate the out-of-sample accuracy of the prediction model.
...
An initial exploratory analysis showed that the classes to be
predicted does not seem linearly separable, and the variable
distributions depart considerably from the normal distribution. The
plot shown below illustrates some of these characteristics for a
sample of the variables corresponding to the Euler angles of the
dumbbell sensor.
...
Considering these characteristics and the main goal of the accuracy of
the predictions, I decided to use random forests to build the
prediction model. The following code fragment builds a random forest
with 500 trees on the training portion of the data, using 10-fold
cross validation with accuracy as the evaluation metric to estimate
the other model parameters (i.e. `mtry`).
...
The construction of the random forest model took `r round(model$times$everything[3] / 60)`
minutes using 4 processes on an Intel(R) Core(TM) i5-4200M CPU at 2.50
GHz with 8 GB of RAM. The selected model was `r round(sum(diag(confusionMatrix(model)$table)), 1)`%
accurate on the training sample. In the next section, the out-of-sample
accuracy is estimated on the separate testing portion of the data set.
...
The random forest model obtained an accuracy of
...
during the construction of the model. Given the high accuracy results,
it is reasonable to expect that the model will perform well on unseen
data.

# Submission Results

Finally, the following code generates the predicted values for the
hold-out sample of the assignment. All the predicted values were
correct.
